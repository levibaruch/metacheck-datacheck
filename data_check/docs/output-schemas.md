...
| `software` | Program or application whose purpose is to **run the experiment** — stimulus delivery tools, task presentation programs, data collection applications, compiled binaries (`.exe`, `.app`, `.jar`, `.msi`, `.dmg`), and installers. Distinct from `code` (which generates analyses/outputs). Notebooks (`.ipynb`, `.Rmd`, `.qmd`) are always `code`, never `software`. |
