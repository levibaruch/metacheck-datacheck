...
2. **Given** a paper where the codebook was not found or not parsed, **When** the report is run, **Then** that paper is shown with coverage = 0% and a note indicating no codebook.
...
- **FR-005**: The report MUST read `outputs/<paper_id>/codebook_coverage.csv` files and produce per-paper and aggregate codebook coverage rates.
...
- **FR-010**: Report output MUST be written to the console in human-readable tabular format; the researcher may optionally also save a summary CSV by passing an output path argument.
